...
function loadProfile(user, profile) {
  ...
  // Language Resources Editor (Vendors & Companies)
  var langResSection = document.getElementById("lang-resources-section");
  if (langResSection) {
    if (profile.role === "vendor" || profile.role === "company") {
      langResSection.style.display = "";
      renderLangResources(profile.languageResources || []);
    } else {
      langResSection.style.display = "none";
    }
  }

  // Toggle lang resources section on account type change
  var typeSelForLang = document.getElementById("pf-type");
  if (typeSelForLang && langResSection) {
    typeSelForLang.addEventListener("change", function() {
      if (this.value === "vendor" || this.value === "company") {
        langResSection.style.display = "";
      } else {
        langResSection.style.display = "none";
      }
  ...
  form.addEventListener("submit", async function (e) {
    ...
    try {
      ...
      // Save language resources for vendors/companies
      if (updateData.role === "vendor" || updateData.role === "company") {
        updateData.languageResources = collectLangResources();
      }
...
async function loadMyProjects(user, profile) {
  ...
  try {
    unsubMyProjects = db.collection("participations").where("userId", "==", user.uid).onSnapshot(async function(snap) {
      if (snap.empty) {
        container.innerHTML = '<p class="section-copy">You do not have any applications yet. Browse open jobs to get started.</p><a class="btn btn-outline" href="jobs.html">Browse open jobs</a>';
        return;
      }
...
// ---------------------------------------------------------------------------
// Language Resources Editor Helpers (Vendors & Companies)
// ---------------------------------------------------------------------------
function renderLangResources(resources) {
  var list = document.getElementById("lang-resources-list");
  var addBtn = document.getElementById("btn-add-lang-resource");
  if (!list) return;
  list.innerHTML = "";

  if (resources && resources.length > 0) {
    resources.forEach(function(r, i) {
      addLangResourceRow(list, r.language || "", r.count || 1);
    });
  }

  if (addBtn) {
    addBtn.onclick = function() {
      addLangResourceRow(list, "", 1);
    };
  }
}

function addLangResourceRow(container, lang, count) {
  var row = document.createElement("div");
  row.className = "lang-resource-row";
  row.innerHTML =
    '<input type="text" class="lr-lang" value="' + esc(lang) + '" placeholder="Language name (e.g., Hindi)">' +
    '<input type="number" class="lr-count" value="' + (count || 1) + '" min="1" placeholder="Count">' +
    '<button type="button" class="btn-remove-lang">✕ Remove</button>';

  row.querySelector(".btn-remove-lang").addEventListener("click", function() {
    row.remove();
  });
...
function collectLangResources() {
  var rows = document.querySelectorAll(".lang-resource-row");
  var result = [];
  rows.forEach(function(row) {
    var lang = row.querySelector(".lr-lang").value.trim();
    var count = parseInt(row.querySelector(".lr-count").value) || 1;
    if (lang) {
      result.push({ language: lang, count: count });
    }
  });
  return result;
}
