...
// ---------------------------------------------------------------------------
// Super Admin allow-list. Only these emails can grant/revoke admin access to
// other users. This MUST stay in sync with isSuperAdmin() in firestore.rules.
// Regular admins (users with role "admin") have full panel access but CANNOT
// promote or demote other admins.
// ---------------------------------------------------------------------------
const SUPER_ADMIN_EMAILS = ["info.yugmai@gmail.com"];
...
function isSuperAdminEmail(email) {
  ...
  return SUPER_ADMIN_EMAILS.some(function (a) { return a.toLowerCase() === e; });
}
...
// Preserve the selected job when a visitor signs in to apply. Only accept an
// in-site job application path so the login flow cannot become an open redirect.
function getAuthNextUrl() {
  ...
  try {
    ...
    if (target.origin !== window.location.origin || !/\/job-apply(\.html)?$/.test(target.pathname) || !(target.searchParams.get("id") || target.searchParams.get("jobId"))) return "";
...
function registrationCompletionUrl() {
  var next = getAuthNextUrl();
  return next ? "/register?complete=1&next=" + encodeURIComponent(next) : "/register?complete=1";
}
...
function guardPage(requirement, onReady) {
  onAuthChange(async (user) => {
    ...
    // Expose super admin flag for admin.js to conditionally show admin management UI
    window._isSuperAdmin = isSuperAdminEmail(user.email);
