...
app.post("/api/contact", async (req, res) => {
  const { name, email, phone, subject, message, type, service, recaptchaToken } = req.body || {};
  ...
  if (process.env.RECAPTCHA_SECRET_KEY) {
    try {
      ...
      if (!verifyData.success || verifyData.score < 0.5) {
        console.warn("[contact] reCAPTCHA failed:", verifyData);
        return res.status(403).json({ error: "Spam check failed. Please try again or email info@yugmai.in directly." });
      }
    } catch (err) {
      console.warn("[contact] reCAPTCHA verification error:", err.message);
...
// Delete file from drive — ownership verified
app.post("/api/drive/delete", requireAuth, async (req, res) => {
  try {
    const { fileId } = req.body;
    if (!fileId) return res.status(400).json({ error: "No fileId provided" });
    
    // Verify the requesting user owns this file or is admin
    const userDoc = await firebaseAdmin.firestore().collection("users").doc(req.user.uid).get();
    const userData = userDoc.exists ? userDoc.data() : {};
    const isAdmin = userData.role === "admin";
    
    if (!isAdmin) {
      // Check if the fileId appears in the user's own profile (CV, etc.)
      const ownsFile = userData.cvUrl && userData.cvUrl.includes(fileId);
      if (!ownsFile) {
        // Also check the user's participations for NDA/invoice files
        const partsSnap = await firebaseAdmin.firestore().collection("participations")
          .where("userId", "==", req.user.uid).get();
        let foundInParts = false;
        partsSnap.forEach(function(doc) {
          const d = doc.data();
          if ((d.ndaUrl && d.ndaUrl.includes(fileId)) ||
              (d.invoiceUrl && d.invoiceUrl.includes(fileId))) {
            foundInParts = true;
          }
        });
        if (!foundInParts) {
